...
| 3 | Packet lore renders on an RPG item | hold/inspect an RPG item | Lore shows rarity+quality, stats, requirement lines, item level and durability through `PacketItemRenderer`, per viewer; other viewers see per-locale lore; vanilla item unchanged (the cooldown line stays deferred — no item→ability link exists yet) | same; packet send anchored to the viewer's context | ☐ |
...
| 10 | Armor set bonus activates at member-count thresholds (WP-5.3) | equip a 4-piece set | the set bonus activates in stats and its lore shows `Name (active/total)` with reached thresholds lit and higher ones dimmed | same; the set-count recompute runs on the equipping player's region thread with no thread error | ☐ |
| 11 | Deferred lore is visible per viewer (wire-deferred-lore) | hover an item below your level, then level the gating skill / equip a partial set | requirement line is red for a viewer who fails it and green for one who meets it (met/unmet is per-viewer); a 2/4 set shows `Name (2/4)` with the 2-piece threshold lit and the 4-piece dimmed; item level, durability and quality lines are present | same; per-viewer state is cached per viewer-state, so a skill-up or armor swap updates the lore with no stale render and no unrelated cache flush | ☐ |
